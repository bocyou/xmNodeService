...
Page({
  ...
  checkDinnerStatus:function(){
    ...
    self.restData();
    util.request({
      url: 'http://localhost:3000/order_food/get_dinner_list', param: '', complete: function (res) {
          ...
          if (list_data.isDraw==1){
           //已刮卡
            if (JSON.stringify(list_data.result) == '{}') {
              ...
              //订餐已结束
              util.request({
                url: 'http://localhost:3000/order_food/check_currentuser_dinner', complete: function (res) {
                  wx.hideLoading();
                  var data = res.data;
  ...
  cancelDinner:function(){
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          util.request({
            url: 'http://localhost:3000/order_food/cancel_currentuser_dinner', param: { menu_id: user_today_id }, complete: function (res) {
              var data = res.data;
  ...
  submit:function(){
    ...
    if (result.length>0){
      ...
      //保存订餐信息
      util.request({
        url: 'http://localhost:3000/order_food/save_user_dinnerlist', param: { dinner_list: JSON.stringify(result)}, complete: function (res) {
          var data = res.data;
          if (data.code == 200&&data.result==true) {
             //订餐成功进入二维码页面
            self.checkDinnerStatus();
