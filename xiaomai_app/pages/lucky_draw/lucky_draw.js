...
Page({
  ...
  getTopList:function(){
    var self = this;
    util.request({
      url: 'http://localhost:3000/lucky_draw/month_top_list', complete: function (res) {
        var data = res.data;
        console.log(data);
  ...
  getSpecialList:function(){
    var self=this;
    util.request({
      url: 'http://localhost:3000/lucky_draw/get_user_special_list', complete: function (res) {
        var data = res.data;
        console.log(data);
        if (data.code == 200 && data.result) {
          console.log(data.result);
  ...
  getAllList:function(){
    var self=this;
    util.request({
      url: 'http://localhost:3000/lucky_draw/get_user_draw_list', complete: function (res) {
        var data = res.data;
        console.log(data);
        if (data.code == 200 && data.result) {
          console.log(data.result);
  ...
  checkCurrentUserDraw:function(){
    var self=this;
    util.request({
      url: 'http://localhost:3000/lucky_draw/check_current_user_draw', complete: function (res) {
        var data = res.data;
        console.log(data);
        wx.hideLoading();
        if(data.code==200&&data.result){
  ...
  startDraw:function(){
    var self=this;
    util.request({
      url: 'http://localhost:3000/lucky_draw/save_user_draw', complete: function (res) {
        var data = res.data;
        console.log(data);
        if(data.code==200){
          self.getAllList();
