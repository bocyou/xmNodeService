...
const work={
    ...
    updateShakeNum(connection,clients,data){
        ...
        shakeInfo(connection,(term_info)=>{
            ...
            console.log(current_term_info.shake_num,current_term_info.user_shake_num);
                ...
                mysql.sql('update shake set user_shake_num=user_shake_num+1 WHERE is_use=1 AND status=1', function (err, result) {
                    if (err) {
                        ...
                        if(current_term_info.shake_num==current_term_info.user_shake_num){
                            ...
                            clients.forEach(function(ws1){
                                ws1.sendUTF(JSON.stringify({result:current_term_info ,type:'current_term_info', code: 200, message: `更新数据`}))
                            })
                        }
            ...
            mysql.sql( 'SELECT * FROM custom_session tab1 JOIN users tab2 ON tab1.open_id = tab2.open_id WHERE '+user_session_str, function (err, result) {
                if(result&&result.length>0){
                    ...
                    mysql.insert_more('shake_win_user(`user_id`, `term`,`create_time`)',[win_users], function ( result,err) {
                        if (err) {
                            connection.sendUTF(JSON.stringify({ type:'error', message: "存储中奖用户失败"}))
                            ...
                            mysql.sql('update shake set status=0,over_time="'+ new Date().Format('yy-MM-dd HH:mm:ss') + '" WHERE is_use=1', function (err, result) {
                                ...
                                if (err) {
                                    clients.forEach(function(ws1){
                                        ws1.sendUTF(JSON.stringify({ type:'error', message: "结束失败（更新shake失败）"}))
                                    })
                                } else {
                                    current_term_info.status=0;
                                    clients.forEach(function(ws1){
                                        ws1.sendUTF(JSON.stringify({result:current_term_info ,type:'current_term_info', code: 200, message: `本期已结束`}))
                                    })
                    ...
                    connection.sendUTF(JSON.stringify({ type:'error', message: "更新中奖用户失败"}))
        ...
        if (err){
            ...
            mysql.sql('update shake set is_use=0 WHERE id='+current_info.id, function (err, result) {
                if (err) {
                    ...
                    mysql.insert_one('shake', {term:parseInt(current_info.term)+1,is_use:1,create_time:new Date(),over_time:'',shake_num:screen_data.shake_num,user_shake_num:0,status:1,win_users:screen_data.win_users}, function (result, err) {
                        if (err) {
