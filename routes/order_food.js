...
router.post('/start_dinner_new', checkSession, function (req, res, next) {
    ...
    //先检查是否开启定餐
    mysql.sql('SELECT * FROM order_fooding  WHERE status="start"', function (err, result) {
        if (err) {
            console.log('获取订餐信息失败');
        } else {
            if (result && result.length > 0) {
                //已开启订餐
                res.status(200).send({
...
router.post('/finish_dinner', checkSession, function (req, res, next) {
    res.header("Access-Control-Allow-Origin", "*");
    mysql.sql('update order_fooding set status="over",over_time="' + new Date().Format('yy-MM-dd HH:mm:ss') + '" where status=start', function (err, result) {
        if (result) {
            res.status(200).send({
                code: 200,
                result: true,
...
router.post('/save_user_dinnerlist', function (req, res, next) {
    getUserInfo(req, res, function (userInfo) {
        if (userInfo) {
            let dinner_list = req.body.dinner_list;
            let spread_money = req.body.spread_money;
            //先获取当前正在进行的菜单
            mysql.sql('SELECT * FROM order_fooding  WHERE status="start"', function (err, result) {
                if (err) {
                    res.status(200).send({
                   ...
                   if(result&&result.length>0){
                       let fooding_id=result[0].id;

                       mysql.sql('SELECT * FROM order_food_user WHERE  AND order_fooding_id="'+fooding_id+'" status = 1 AND user_id=' + userInfo[0].id, function (err, result) {
                           if (err) {
                               res.status(200).send({
                                   code: 500,
                                   result: false,
...
router.post('/check_currentuser_dinner', function (req, res, next) {
    getUserInfo(req, res, function (userInfo) {
        ...
        if (userInfo) {
            ...
            //获取当前正在进行的菜单，然后通过ID找到记录判断状态
            mysql.sql('SELECT * FROM order_fooding  WHERE status="start"', function (err, result) {
                if (err == null) {
                    res.status(200).send({
                        code: 200,
                        result: [],
                        message: '获取正在进行的订餐列表失败'
                    });
                } else {
                    let fooding_id=result[0].id;
                    mysql.sql('SELECT * FROM order_food_user WHERE order_food_user="'+fooding_id+'" AND status=1 And user_id=' + user_id, function (err, result) {
                        if (err == null) {
                            if (result.length > 0) {
                                res.status(200).send({
                                    code: 200,
