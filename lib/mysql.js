...
module.exports = {
    ...
    updateOne:(table,id,params,callback)=>{
        let params_key=[];
        let  params_val=[];
        for (let key in params){
            params_key.push(`${key}=?`);
            params_val.push(params[key]);
        }
        params_val.push(id);
        console.log(params_key.join(','),params_val);
        connection.query(`UPDATE ${table} SET ${params_key.join(',')} WHERE id = ?`, params_val, function (err, result,fields) {
    ...
    findWeek:function(table,condition,callback){
        connection.query('SELECT * FROM '+table+' WHERE YEARWEEK(create_time,1)=YEARWEEK(now(),1) AND '+condition,
            function(err, result) {
                callback(result, err);
            }
        );
    ...
    findCurrentWeek:function(table,condition,callback){
        connection.query('SELECT * FROM '+table+' WHERE YEARWEEK(create_time,1) = YEARWEEK(now(),1)'+condition,
            function(err, result) {
                callback(result, err);
            }
        );
